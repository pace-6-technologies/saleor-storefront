...
import { uploadSlipQuery } from "./queries";
...
const fetchItems = async ({ query, variables = {} }, callback: any) => {
  const client = new ApolloClient({
    ...
    link: createUploadLink({ uri: API_URL, fetch }),
  });
  const next = async () => {
...
export const uploadSlipPayment = async ({ params, callback }) => {
  await fetchItems({ query: uploadSlipQuery, variables: params }, res => {
    callback(res);
  });
};
